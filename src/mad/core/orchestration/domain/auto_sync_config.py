...
Auto-sync is the second, unrequested agent run Mad fires after every primary
run: it publishes whatever uncommitted/unpushed work is left in the workspace
(see ``mad.core.sessions.use_cases.auto_sync_prompt``). It exists as a safety
net so an ad-hoc session cannot silently lose work.

For a task that already manages its own named branch/PR, that safety net is pure
harm: auto-sync has no visibility into which branch the task was told to use, so
it defaults to ``mad/<session_id>`` and opens a **duplicate** PR alongside the
real one (issue #109). This module is the deterministic opt-out — a boolean gate
resolved in Mad, *not* an instruction we hope the agent obeys.
...
def resolve_effective_auto_sync(
    ...
    (``True``) when every level is unset. Always returns a concrete bool — every
    run either syncs or does not; there is no "omit" sentinel (unlike
    model/effort, where ``None`` means "let the provider decide").

    ``False`` is a *value*, not an absence: an explicit ``auto_sync=False`` at
    the task level wins over an ``auto_sync=True`` session, which is the whole
    point of the knob. Hence the ``is not None`` test rather than a truthiness
    check.
    """
...
def env_auto_sync() -> bool | None:
    ...
    recognised — so a typo'd value reports ``None`` here and the safety net stays
    on rather than being silently disabled.
    """
