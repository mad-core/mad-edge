...
| `MAD_AUTO_SYNC` | Toggle for post-run auto-sync: when enabled, after an agent run Mad fires a second run that publishes uncommitted work to a `mad/<session_id>` branch and opens a PR. **Off by default** (#109) — it is opt-in, because auto-sync cannot see a branch/PR a task manages itself and would open a duplicate; set to `true` to enable it deployment-wide as a "don't lose work" net. Overridable per session (`auto_sync` in create-session) and per task (`auto_sync` in enqueue-task); precedence is task > session > this var > false. Accepts true/false, 1/0, yes/no, on/off (case-insensitive); malformed/empty values silently revert to the default (off). | bool | `false` | No | `settings.load_settings().auto_sync`; default consulted during auto-sync orchestration (per-session/task override resolution) |
