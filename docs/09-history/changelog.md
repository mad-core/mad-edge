...
The package's released versions and what changed in each, mirrored from the semantic-release `CHANGELOG.md` at the repo root. Only released, version-tagged changes a `mad-edge` consumer inherits (hard rule 12) — NOT a raw git log.

This is the consumer-facing release history of the `mad-edge` package (import package `mad`; published as `mad-bros` before v0.6.0). It is generated by python-semantic-release from Conventional Commits on the `main` branch (`[tool.semantic_release]` in `pyproject.toml`), tagged `v{version}`, and grouped per released version into sections such as Features and Bug Fixes. Per hard rule 12, only public-scope changes — `http`, `sse`, `cli`, `config`, `agents`, `deps` — are surfaced to consumers; internal `refactor` / `chore` / `test` commits still bump the version but are filtered out of the changelog by `exclude_commit_patterns`. The canonical source is `/CHANGELOG.md` at the repo root; the entries below mirror it verbatim.
